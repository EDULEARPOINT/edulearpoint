...
export const GlobalContextProvider = ({ children }) => {
      ...
      if (referrerId) {
        ...
        while (currentReferrerId && currentLevel < 5) {
          ...
          if (upperReferrerDoc.exists()) {
            ...
            if (currentLevel > 0) {
              // This is a passive member (level 1 and above)
              await updateDoc(upperReferrerRef, {
                [`passiveTeamMembers.level${currentLevel}`]: arrayUnion(
                  user.uid
                ),
              });
            }
            ...
            currentLevel++;
            currentReferrerId = upperReferrerData.referredBy
              ? upperReferrerData.referredBy.userId
              : null;
          } else {
            break;
          }
    ...
    try {
      for (let i = 0; i < referralChain.length; i++) {
        ...
        if (referrerDoc.exists()) {
          ...
          const packageData =
            commissionStructure[referrer.package] || commissionStructure.bronze;
  ...
  const getReferralChain = async (userId) => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      if (userDoc.exists()) {
        ...
        console.log(
          `User document not found for ID: ${currentUser.referredBy.userId}`
        );
        break;
      }
    }
  ...
  const updateReferralChainAndCommissions = async (userId) => {
    try {
      ...
      console.log(
        "Starting referral chain and commission update for user:",
        userId
      );
  ...
  const fetchTeamMembers = async (user, userDetails) => {
    ...
    try {
      ...
      if (userDetails.directTeamMembers) {
        const directMemberPromises = userDetails.directTeamMembers.map(
          async (memberId) => {
        ...
        allMembers = allMembers.concat(
          directMembers.filter((member) => member !== null)
        );
      }
      ...
      if (userDetails.passiveTeamMembers) {
        for (let level = 1; level <= 5; level++) {
          const levelMembers =
            userDetails.passiveTeamMembers[`level${level}`] || [];
          ...
          allMembers = allMembers.concat(
            passiveMembers.filter((member) => member !== null)
          );
        }
      }
  ...
  const fetchAllEnquiries = async () => {
    try {
      ...
      for (const doc of userSnapshot.docs) {
        const userData = doc.data();
        if (
          userData.studentEnquiryDetails &&
          userData.studentEnquiryDetails.length > 0
        ) {
  ...
  const exportedFunc = {
    ...
    getUserKYCData,
    updateCommissions,
    updateReferralChainAndCommissions,
    fetchAllUsers,
